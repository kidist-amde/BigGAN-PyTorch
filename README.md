...
The models are trained for the same number of iterations(with the same batch size). The scores seem very close, which suggests that the BigGAN model learns to generate realistic images even when we have smaller sets. In addition, the images generated from the two models also look similar. I have attached the images generated by the two models.
